...
```
4. Added the game number (so that we can group it later in the plot) to the data frame. Following which, pivoted it to a long data frame for ease of plotting
...
plot <- walks %>% ggplot(aes(x=Round,y=Money,group=factor(game),color=factor(game))) +
  ...
  theme_bw() 
```
![Rplot](https://user-images.githubusercontent.com/68678549/97037265-a195df00-159b-11eb-9ee6-c9d9a7b51ee0.png)
...
##[1] 0.39821
```
6. For this question, I struggled for awhile wondering how to handle the conditional probability. I then realised that the conditional probability could be embedded within the function since S would "-3" given that you lost the first 3 bets
